...
def init_jinja2(app, **kw):
    logging.info('init_jinja2 enter...')
    options  = dict(
...
async def reponse_factory(app, handler):
    async def response(request):
        ...
        if isinstance(r, tuple) and len(r) == r:
            t, m = r
...
async def init(loop):
    await orm.create_pool(loop=loop, host='127.0.0.1', port=3306, user='root', password='cmc123456', db='awesome')
    app = web.Application(loop=loop, middlewares=[
        logger_factory, reponse_factory
    ])
    # 初始化 jinjia2，同时注册filters用于模板中使用
